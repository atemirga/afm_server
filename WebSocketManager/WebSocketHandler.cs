...
namespace WebSocketManager
{
    public abstract class WebSocketHandler
    {
        ...
        public WebSocketHandler(WebSocketConnectionManager webSocketConnectionManager, MethodInvocationStrategy methodInvocationStrategy) : this(webSocketConnectionManager, methodInvocationStrategy, null)
        {
        }
        ...
        /// <param name="webSocketConnectionManager">The web socket connection manager.</param>
        /// <param name="methodInvocationStrategy">The method invocation strategy used for incoming requests.</param>
        public WebSocketHandler(WebSocketConnectionManager webSocketConnectionManager, MethodInvocationStrategy methodInvocationStrategy, ILogger logger)
        {
        ...
        public async Task Away(WebSocket socket)
        {
            var socketId = WebSocketConnectionManager.GetId(socket);
            var groupId = WebSocketConnectionManager.GetSocketGroup(socketId);

            WebSocketConnectionManager.Groups[groupId][socketId].Away = true;
        }

        public async Task AwayBack(WebSocket socket)
        {
            var socketId = WebSocketConnectionManager.GetId(socket);
            var groupId = WebSocketConnectionManager.GetSocketGroup(socketId);

            WebSocketConnectionManager.Groups[groupId][socketId].Away = false;
        }
        ...
        {
            var webSocket = WebSocketConnectionManager.GetSocketById(socketId);
            try
            {
        ...
        public async Task ReceiveAsync(WebSocket socket, WebSocketReceiveResult result, Message receivedMessage)
        {
            ...
            if (receivedMessage.MessageType == MessageType.MethodInvocation)
            {
                ...
                else
                {
                    ...
                    if (invokeResult.Result is Task)
                    {
