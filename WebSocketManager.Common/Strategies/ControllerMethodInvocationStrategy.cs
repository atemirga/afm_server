        ...
        public override async Task<object> OnInvokeMethodReceivedAsync(WebSocket socket, InvocationDescriptor invocationDescriptor)
        {
            ...
            for (int i = 0; i < args.Count; i++)
            {
                ...
                if (argValue != null)
                {
                    ...
                    var parameterType = parameterTypes[i + 1].ParameterType;
                    if (argType.IsPrimitive || argType == typeof(string))
                    {
                        if (argType != parameterType)
                        {
                            args[i] = Convert.ChangeType(argValue, parameterType);
                        }
                    }
                    else
                    {
                        args[i] = JsonConvert.DeserializeObject(args[i].ToString(),
                            parameterType, _jsonSerializerSettings);
                    }
                }
            }
