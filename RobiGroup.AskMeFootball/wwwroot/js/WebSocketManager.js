...
var WebSocketManager = (function () {
    ...
    var constructor = function (url) {
        ...
        var onSocketMessage = function (message) {
            ...
            // METHOD INVOCATION EVENT
                ...
                if (_this.methods[data.methodName] !== undefined) {
                    ...
                    try { result = _this.methods[data.methodName].apply(_this, parseCSharpArguments(data.arguments)); }
                    catch (e) { error = e; }
                    ...
                    if (data.identifier !== '00000000-0000-0000-0000-000000000000') {
                        ...
                        if (error !== undefined) {
                            ...
                            _this.socket.send(JSON.stringify(new Message(Message.MethodReturnValue,
                                JSON.stringify(new InvocationResult(data.identifier, null, "A remote exception occured: " + error))
                            )));
                        }
                        // send result value to the server.
                            ...
                            // send web-socket message to the server.
                            _this.socket.send(JSON.stringify(new Message(Message.MethodReturnValue,
                                JSON.stringify(new InvocationResult(data.identifier, { $type: result[0], $value: result[1] }))
                            )));
                        }
                    }
            ...
            // METHOD RETURN VALUE EVENT
                ...
                delete waitingRemoteInvocations[data.identifier];
                // call the callback.
        ...
        this.invoke = function (method) {
            ...
            // send web-socket message to the server.
            var message = new Message(Message.MethodInvocation,
